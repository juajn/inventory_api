...
@router.post("/", response_model=ProductOut, dependencies=[Depends(get_current_superuser)])
...
@router.get("/", response_model=List[ProductOut], dependencies=[Depends(get_current_active_user)])
def list_products(skip: int = 0, limit: int = 100, db: Session = Depends(get_db_safe)):
    return get_products(db, skip=skip, limit=limit)


@router.get("/{product_id}", response_model=ProductOut, dependencies=[Depends(get_current_active_user)])
...
@router.put("/{product_id}", response_model=ProductOut, dependencies=[Depends(get_current_superuser)])
...
@router.delete("/{product_id}", dependencies=[Depends(get_current_superuser)])
